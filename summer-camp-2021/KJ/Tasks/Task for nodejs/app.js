...
http.createServer(
    function(req, res) {
        ...
        fs.readFile('./Task for todolist' + pathname, function(err, data) {
            if (err) {
...
console.log("http://127.0.0.1:9521 Listening...");*/
                ...
                fs.readdir(finalpath, function(err, files) {
                    ...
                    //将文件结构写入页面
                        ...
                        for (var i in files) {
                            ...
                            html += "<div><p>|-<a href='http://127.0.0.1:9521" +
            ...
            //如果是文件
                ...
                fs.readFile(finalpath, function(err, data) {
                    if (err) res.end("cannot read file!");
                        ...
                        res.writeHead(200, { 'Content-Type': '' + mime + ';charset=utf-8' });
                        //res.writeHead(200, { "Content-Type": mime.getType(path.basename(pathname)) });
                        res.end(data);
                    }
                })
            } else {
